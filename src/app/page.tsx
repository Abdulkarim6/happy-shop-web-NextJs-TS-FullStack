import Banner from "./components/homeComponents/Banner/Banner";
import Categories from "./components/homeComponents/Categories/Categories";
import CustomerBenefits from "./components/homeComponents/CustomerBenefits/CustomerBenefits";
import NewArrivals from "./components/homeComponents/NewArrivals/NewArrivals";

export default function Home() {
  return (
    <section className="flex flex-col justify-center">
      <Banner />
      <CustomerBenefits />
      <Categories />
      <NewArrivals/>

      <h1 className="my-5"> ed cum dolore ipsa fuga doloribus deserunt impedit quibusdam earum perferendis inventore obcaecati omnis minus? Sequi culpa harum nulla nisi qui non.
      Veritatis tempora dolorepe vel adipisci quasi atque necessitatibus inventore suscipit officiis eligendi impedit! Perspiciatis, assumenda eaque consequatur eius veritatis quibusdam quidem odio excepturi alias laborum.
      Non possimussequi nemo magni pariatur modi sunt, quae veritatis eum officia ullam debitis dolorum quam id magnam tenetur doloremque necessitatibus dignissimos libero qui. Quaerat doloribus quis repudiandae.
      Sapiente commodi id iste molestias, iure qui earum cumque voluptate, mollitia assumenda perferendis nam impedit facilis molestiae dolorum pariatur reprehenderit nemo vero consectetur quidem. Error aperiam velit commodi deleniti minima.
      Repellat in voluptatum maiores recusandae eos maxime eaque sint sapiente similique adipisci quidem, aliquam voluptates. Expedita, delectus repellendus? Quisquam, inventore. Reiciendis aperiam quasi pariatur dolorum assumenda repellendus. Consectetur, nemo in.
      Numquam, rem eveniet dolores alias maiores pariatur deleniti distinctio provident fuga, laboriosam modi minus asperiores fugit vel repellendus id, quas voluptatibus voluptates eligendi voluptas consequatur architecto quos. Incidunt, placeat doloremque?
      Fugit dolorum asperiores officiis dolore, nemo in, temporibus amet minima iure veniam ex quo odit. Tempora ex eum ab provident, eius voluptate odit, unde aspernatur distinctio in molestias, harum libero!
      Neque necessitatibus vitae iusto veniam natus accusamus, ratione commodi laboriosam consequuntur illo hic aspernatur dolor veritatis ab aliquam, doloribus voluptatibus repellendus vero consectetur perspiciatis quis esse! Rem odio corporis quasi.
      At eum nulla ad, officiis dolorum quos qui aperiam, aliquam optio dolorem atque nemo quaerat facilis? Magnam blanditiis eveniet enim aliquam maxime animi dictam, tempora eaque laboriosam quasi quae placeat eveniet, earum nobis.
      Nesciunt, et sequi? Doloremque odit odio quidem aperiam. Sit cupiditate labore quis illum qui quidem. Accusantium quas reiciendis aliquam quaerat, voluptates repellendus animi illo voluptatum mollitia, repudiandae sed itaque rerum?</h1>
    </section>
  );
}
